...
// Fixed stop/target distance in absolute price units, used when ATR risk mode is disabled.
#define DEFAULT_FIXED_MOVE 1.44

// Training labels: ATR multipliers used only when the trainer runs with -r / ATR risk mode enabled.
#define LABEL_SL_MULTIPLIER 0.01
#define LABEL_TP_MULTIPLIER 0.54

// Live execution defaults used only when ATR risk mode is enabled.
