...
sys.path.append(os.getcwd()) # For Colab root if uploaded flat

try:
    from shared_mamba import SharedMambaClassifier
except ImportError:
    print("[ERROR] Cannot import shared_mamba. Please ensure shared_mamba.py is uploaded to Colab.")
    sys.exit(1)
...
def main():
    if IN_COLAB:
        ...
        save_dir = '/content/drive/MyDrive/fx'
        os.makedirs(save_dir, exist_ok=True)
        
        # Check if data exists in Drive, otherwise fallback to local Colab storage
        data_path = os.path.join(save_dir, DATA_FILE)
        if not os.path.exists(data_path):
            print(f"[WARN] {DATA_FILE} not found in {save_dir}. Looking in current directory.")
            data_path = DATA_FILE
        
        onnx_path = os.path.join(save_dir, "gold_mamba.onnx")
        
        # In Colab, we can use larger batch sizes and model capacities thanks to the GPU
        batch_size = 128
        d_model = 128
        hidden = 256
    else:
    ...
    # ─────────────────────────────────────────────────────────────
    if not os.path.exists(data_path):
        print(f"[FATAL] Data file {data_path} not found. Please upload it.")
        return

    bars_by_symbol = build_aligned_bars(data_path, SYMBOL_ORDER, TICK_DENSITY)
    ...
    # 8. MAMBA MODEL
    # ─────────────────────────────────────────────────────────────
    model     = SharedMambaClassifier(n_features=N_FEATURES, d_model=d_model, hidden=hidden).to(device)
